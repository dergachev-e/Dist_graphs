...
void Print_Dist_Graph(vector <Node> G, int &number, double a, double b) {
    ...
    if (Edges.size() > 26750) {
        ...
        lout << number << " " << a << " " << b << endl;
        lout.close();
        number++;
    }
    string s = to_string(number);
    cout << s << G.size() << " " << Edges.size() << endl;
    ofstream fout;
...
int main() {
    ...
    U1.push_back(turn(sqrt(11.0) / (2.0 * sqrt(3.0)), -1.0 / (2.0 * sqrt(3.0)), Node(1.0, 0.0)));
    double bb = 3 * (1 - sqrt(2.0 / 3.0)) / 2;
    U1.push_back(turn(bb, sqrt(1 - bb*bb), Node(1.0, 0.0)));
    U1.push_back(turn(bb, -sqrt(1 - bb*bb), Node(1.0, 0.0)));
    U1 = turn60(U1);
    ...
    for (set < pair< int, pair <double, double > > >::iterator it = angles.begin(); it != angles.end(); ++it) {
        ...
        Print_Dist_Graph(G, cnt, it->second.first, it->second.second);
    }
}